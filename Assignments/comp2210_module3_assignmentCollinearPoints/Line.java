...
public class Line implements Comparable<Line>, Iterable<Point> {
   ...
   public Line(Collection<Point> c) {
      ...
      Iterator<Point> iter = c.iterator();
      while(iter.hasNext()) {
         add(iter.next());
      }
   }
   ...
   public boolean add(Point p) {
      if (line.contains(p)) {
         return false;
      } else if ((line.isEmpty()) || (line.size() == 1) || (line.first().slopeTo(p) == line.last().slopeTo(p))) {
         line.add(p);
         return true;
      }
      return false;
   ...
   public int compareTo(Line that) {
      if (line.isEmpty() && that.length() == 0) {
         return 0;
      } else if (!line.isEmpty() && that.length() == 0) {
         return 1;
      } else if (line.isEmpty() && that.length() != 0) {
         return -1;
      }
      ...
      if (line.first().compareTo(that.first()) < 0) {
         return -1;
      } else if ((line.first().compareTo(that.first()) == 0) && (line.last().compareTo(that.last()) < 0)) {
         return -1;
      } else if ((line.first().compareTo(that.first()) == 0) && (line.last().compareTo(that.last()) == 0)) {
         return 0;
      } else {
